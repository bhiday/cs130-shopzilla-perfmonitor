...
privileged aspect CompositePerfDataDataOnDemand_Roo_DataOnDemand {
    ...
    public CompositePerfData CompositePerfDataDataOnDemand.getNewTransientCompositePerfData(int index) {
        com.shopzilla.perfmonitor.data.CompositePerfData obj = new com.shopzilla.perfmonitor.data.CompositePerfData();
    ...
    public void CompositePerfDataDataOnDemand.init() {
        data = com.shopzilla.perfmonitor.data.CompositePerfData.findCompositePerfDataEntries(0, 10);
        if (data == null) throw new IllegalStateException("Find entries implementation for 'CompositePerfData' illegally returned null");
        if (!data.isEmpty()) {
            return;
        }
        
        data = new java.util.ArrayList<com.shopzilla.perfmonitor.data.CompositePerfData>();
        for (int i = 0; i < 10; i++) {
            com.shopzilla.perfmonitor.data.CompositePerfData obj = getNewTransientCompositePerfData(i);
            obj.persist();
            obj.flush();
            data.add(obj);
        }
